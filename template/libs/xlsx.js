...
function make_xlsx_lib(XLSX) {
  ...
  function writextag(f, g, h) {
	  if (f === 'td') {
		return '<' + f + ((h != null) ? wxt_helper(h) : "") + ((g != null) ? (g.match(wtregex) ? ' xml:space="preserve"' : "") + 'title="'+ g +' ">' + g + '</' + f : "/") + '>';
	  }
        ...
        switch (y[0]) {
          ...
          default:
            if (x.indexOf('<vt:') === 0) {
              ...
              /* empty */ } else if (opts.WTF) throw new Error(x);
        }
      }
    return p;
  }
  ...
  var PRN = (function () {
    function set_text_arr(data, arr, R, C, o) {
      if (o.raw) arr[R][C] = data;
        ...
        /* empty */ } else if (!isNaN(fuzzynum(data))) arr[R][C] = fuzzynum(data);
      ...
      switch (RT) {
        ...
        default:
          if (R_n.indexOf("Begin") > 0) {
            /* empty */ } else if (R_n.indexOf("End") > 0) {
            /* empty */ }
    ...
    recordhopper(data, function hopper_cc(val, R_n, RT) {
      switch (RT) {
        ...
        default:
          if ((R_n || "").indexOf("Begin") > 0) {
            /* empty */ } else if ((R_n || "").indexOf("End") > 0) {
            /* empty */ } else if (!pass || opts.WTF) throw new Error("Unexpected record " + RT + " " + R_n);
      }
    });
    return out;
  }
    ...
    recordhopper(data, function xlink_parse(val, R_n, RT) {
      ...
      switch (RT) {
        ...
        default:
          if ((R_n || "").indexOf("Begin") > 0) {
            /* empty */ } else if ((R_n || "").indexOf("End") > 0) {
            /* empty */ } else if (!pass || opts.WTF) throw new Error("Unexpected record " + RT.toString(16) + " " + R_n);
      }
    }, opts);
  }
        ...
        case 'PtgNameX':
          ...
          if (opts.biff <= 5) {
            ...
            if (((supbooks[bookidx] || [])[0] || [])[0] == 0x3A01) {
              /* empty */ } else if (((supbooks[bookidx] || [])[0] || [])[0] == 0x0401) {
    ...
    recordhopper(data, function hopper_wb(val, R_n, RT) {
      switch (RT) {
        ...
        default:
          if ((R_n || "").indexOf("Begin") > 0) {
            /* empty */ } else if ((R_n || "").indexOf("End") > 0) {
            /* empty */ } else if (!pass || (opts.WTF && state[state.length - 1] != "BrtACBegin" && state[state.length - 1] != "BrtFRTBegin")) throw new Error("Unexpected record " + RT + " " + R_n);
      }
    }, opts);
          ...
          case 'worksheetoptions' /*case 'WorksheetOptions'*/ :
            switch (Rn[3]) {
              case 'visible' /*case 'Visible'*/ :
                if (Rn[0].slice(-2) === "/>") {
                  /* empty */ } else if (Rn[1] === "/") switch (str.slice(pidx, Rn.index)) {
                  case "SheetHidden":
                    wsprops.Hidden = 1;
                    break;
  ...
  */
  ...
  function write_zip(wb, opts) {
    ...
    if (wb.Props && wb.Props.SheetNames) {
      /* empty */ } else if (!wb.Workbook || !wb.Workbook.Sheets) wb.Props.SheetNames = wb.SheetNames;
    else {
      var _sn = [];
