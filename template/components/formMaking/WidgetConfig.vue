  ...
  <div v-if="show">
    <el-form label-position="top">
      <el-form-item
        label="指标选择"
        v-if="
          Object.keys(data.options).indexOf('indexId') >= 0 &&
          showVal.length > 0
        "
        :required="true"
      >
      ...
      <el-form-item
        label="是否检索字段"
        v-if="Object.keys(data.options).indexOf('searchIndex') >= 0"
        :required="true"
      >
        <el-switch
          v-model="data.options.searchIndex"
          active-color="#13ce66"
          inactive-color="#ff4949"
          @change="addSearchFieldList"
        >
        </el-switch>
      </el-form-item>
      <el-form-item
        ...
        indexRelation
        v-if="
          Object.keys(data.options).indexOf('indexRelation') >= 0 &&
          data.options.indexId != null
        "
      >
        <ConnectionGernerator
          ref="ConnectionGernerator"
          :searchFieldList="searchFieldList"
          :otherIndes="otherIndes"
          :dataList="data.options.indexRelation"
      ...
      <el-form-item label="别名" v-if="data.type != 'grid'">
        <el-input v-model="data.options.alias"></el-input>
      </el-form-item>
      <el-form-item label="字段长度" v-if="data.type != 'grid'">
        <el-input
          v-model="data.fileLength"
          :disabled="data.options.indexId != null"
        ></el-input>
      </el-form-item>
      <el-form-item
        label="宽度"
      ...
      <el-form-item
        label="是否列表展示"
        v-if="Object.keys(data.options).indexOf('visible') >= 0"
      >
        <el-switch
          v-model="data.options.visible"
          :active-value="1"
          :inactive-value="0"
        ></el-switch>
      </el-form-item>
      ...
      <template
        v-if="
          data.type != 'grid' && data.type != 'divider' && data.type != 'title'
        "
      >
...
import ConnectionGernerator from './ConnectionGenerator.vue'
export default {
  components: {
    Draggable, SelectTree, ConnectionGernerator
  },
  props: ['data', 'selectValues'],
  data () {
    return {
      searchFieldList: [],//检索字段list
      otherIndes: null,//关联指标数据
      validator: {
        name: '',
        type: null,
        required: null,
  ...
  methods: {
    // 添加检索字段
    addSearchFieldList (val) {
      if (val) {
        if (this.searchFieldList.length == 0) {
          this.searchFieldList.push({ label: this.data.name, value: this.data.model })
        } else {
          for (let i = 0; i < this.searchFieldList.length; i++) {
            if (this.searchFieldList[i].value == this.data.model) {
              break
            }
            if (i == this.searchFieldList.length - 1) {
              this.searchFieldList[i + 1] = { label: this.data.name, value: this.data.model }
            }
          }
        }
        ...
        for (let i = 0; i < this.searchFieldList.length; i++) {
          if (this.searchFieldList[i].value == this.data.model) {
            this.searchFieldList.splice(i, 1)
          }

        }
      }
    },
    getGeneratorValue (val) {
      console.log(val)
      this.data.options.indexRelation = val
    },
    // 树型下拉框
    getValue (val) {
      this.data.options.indexId = val[0];
      // 根据选择的指标确定其他部分选项
      //标题
      this.data.name = val[1].index_chinese_name
      // 长度
      this.data.fileLength = val[1].index_length
      // 数据绑定Key
      this.data.model = val[1].index_name
      this.data.options.alias = val[1].index_name

      this.getOtherIndexs()
    },
    // 获取关联指标
    getOtherIndexs () {
      // this.otherIndes = [{ label: 'TEST1', value: 1 }, { label: 'TEST2', value: 2 }, { label: 'TEST3', value: 3 }]
    },
    ...
    valiatePattern (val) {
      ...
      if (val) {
        this.validator.pattern = { pattern: val, message: this.data.name + this.data.options.message }
      } else {
    ...
    valiateMessage (val) {
      this.data.options.message = val
      if (this.validator.pattern) {
        this.validator.pattern.message = this.data.name + this.data.options.message
        this.generateRule()
      }
  ...
  watch: {
    'data.options.indexId': function () {
      if (this.data.options.indexId == null) {
        // this.valueId = null
      }
    },
    ...
    'data.options.message': function (val) {
      this.valiateMessage(val)
    },
