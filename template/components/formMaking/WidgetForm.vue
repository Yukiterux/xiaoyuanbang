          ...
          <template v-for="(element, index) in data.list">
            ...
            <template v-else-if="element.type == 'table'">
              <div class="widget-col widget-view" v-if="element && element.key" :key="element.key" 
                 ...
                 <el-table :data="element.tableData" border style="width: 100%" :show-header="false" :row-style="{minHeight: '50px'}">
                      <el-table-column v-for="(col, colIndex) in element.columns" :key="colIndex">
                        <template slot-scope="" >
                          <draggable
                            v-model="col.list"
                            :no-transition-on-drag="true"
  ...
  methods: {
    ...
    handleWidgetAdd (evt) {
      ...
      this.$set(this.data.list, newIndex, {
        ...
        key,
        // 绑定键值 this.data.list[newIndex].type + '_' + key
