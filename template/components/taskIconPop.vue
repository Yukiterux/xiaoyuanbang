<template>
  <el-dialog title="选择图标" :visible.sync="taskIconDialog" width="1200px"
    ...
    <Card dis-hover>
      <template v-for="(item, index) in iconList">
        <div class="icon-box" @click="selected(item)">
          ...
          <img :src="require('../assets/img/rules/' + item + '.png')" alt="" style="width: 30px;height: 30px;display: inline-block;">
...
<script>
  import {iconArray} from '@/assets/taskIcon'
  ...
  export default {
    ...
    methods: {
      ...
      // 选中图标
      selected(iconName) {
        this.$emit('hideTaskIconView', 'false', iconName)
      },
      // 关闭按钮
      closeDialog() {
        this.$emit('hideTaskIconView', 'false')
      }
    },
    created() {
      this.getIconList()
