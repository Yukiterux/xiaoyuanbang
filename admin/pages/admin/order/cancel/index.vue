    ...
    <div class="block">
      ...
      <div class="table">
        ...
        >
          ...
          <el-table-column label="操作"  style="dispaly:inline-block">
            <template slot-scope="scope">
...
import dzdm from "../info/dzdm.json";
export default {
  created() {
    this.search();
  },
  ...
  methods: {
    continueOrder(row) {
      orderApi.continue(row.id).then(res => {
        if (res.data.flag) {
          this.$message({
            message: "修改订单状态成功",
            type: "success"
    ...
    cancel(row) {
      orderApi.cancel(row.id).then(res => {
        if (res.data.flag) {
          this.$message({
            message: "取消成功",
            type: "success"
