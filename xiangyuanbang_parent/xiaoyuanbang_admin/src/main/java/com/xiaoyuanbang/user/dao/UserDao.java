...
public interface UserDao extends JpaRepository<User,String>,JpaSpecificationExecutor<User>{
	...
	@Query(value = "SELECT * FROM tb_user ORDER BY RAND() LIMIT 1",nativeQuery = true)
	User randomUser();
	...
	@Query(value = "update tb_user set nickname=?,sex=?,birthday=?,qq=?,email=? where id=?",nativeQuery = true)
	@Modifying
	void updateInfo(String name, String sex, Date birthday, String qq, String email, String id);
	...
	@Query("select new map(id as id,mobile as mobile,nickname as nickname,sex as sex,birthday as birthday,avatar as avatar,is_errands as is_errands,account_balance as account_balance,grade as grade,qq as qq,email as email) from User where id=?1")
    Map<String,Object> userfindById(String id);
	@Query(value = "select ifnull(count(*),0) from tb_user where is_errands='2' ",nativeQuery = true)
