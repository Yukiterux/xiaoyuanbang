...
public class UserService  {
	...
	private Specification<User> createSpecification(Map searchMap) {
		...
		return new Specification<User>() {
			...
			public Predicate toPredicate(Root<User> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
                ...
                // 
                if (searchMap.get("status")!=null && !"".equals(searchMap.get("status"))&& !"0".equals(searchMap.get("status"))) {
                	predicateList.add(cb.like(root.get("status").as(String.class), "%"+(String)searchMap.get("status")+"%"));
                }
				if (searchMap.get("is_errands")!=null && !"".equals(searchMap.get("is_errands"))&& !"100".equals(searchMap.get("is_errands"))) {
					predicateList.add(cb.like(root.get("is_errands").as(String.class), "%"+(String)searchMap.get("is_errands")+"%"));
				}
                // 
	...
	public void updateInfo(User user) {
		userDao.updateInfo(user.getNickname(),user.getSex(),user.getBirthday(),user.getQq(),user.getEmail(),user.getId());
	}
